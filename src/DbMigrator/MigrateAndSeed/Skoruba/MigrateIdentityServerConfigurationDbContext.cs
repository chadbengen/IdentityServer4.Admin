...
using DbMigrator.MigrateAndSeed.Skoruba.Models;
...
namespace Skoruba.DbMigrator.MigrateAndSeed
{
    ...
    public class MigrateIdentityServerConfigurationDbContext : MigrateAndSeedBase
    {
        ...
        {
            return DoSeed<IdentityServerConfigurationDbContext>(services.BuildServiceProvider(), configurationRoot);
        }

        private async Task DoSeed<TDbContext>(ServiceProvider serviceProvider, IConfigurationRoot configurationRoot)
            where TDbContext : DbContext, IAdminConfigurationDbContext
        {
            using (var scope = serviceProvider.GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                ...
                {
                    IdentityServerDataConfiguration identityServerDataConfiguration = new IdentityServerDataConfiguration();
                    configurationRoot.GetSection("IdentityServerData").Bind(identityServerDataConfiguration);
