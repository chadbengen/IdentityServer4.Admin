...
namespace DbMigrator.MigrateAndSeed.Skoruba
{
    ...
    public class MigrateAdminIdentityDbContext : MigrateAndSeedBase
    {
        ...
        {
            return DoSeed<UserIdentity, UserIdentityRole>(services, configurationRoot);
        }

        private async Task DoSeed<TUser, TRole>(IServiceCollection services, IConfigurationRoot configurationRoot)
            where TUser : UserIdentity, new()
            where TRole : UserIdentityRole, new()
        {
            IdentityDataConfiguration seeddata = new IdentityDataConfiguration();
            configurationRoot.GetSection("IdentityData").Bind(seeddata);
